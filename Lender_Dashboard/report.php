...
function Header()
{
    ...
    $this->Cell(120);
    // Title
    $this->Cell(80,10,'Money Sent Transactions',1,1,'C');
    // Line break
    $this->Ln(20);
}
...
"SELECT  agent_account_number,lent_amount, time_allocated , unique_code FROM lender_transactions where lender_id='$lender_id'") or die("database error:". mysqli_error($connString));
...
// }
$pdf->Ln();
$pdf->Cell(40,12,'Agent_Account_Number' );
$pdf->Cell(40,12,'Lent_Amount');
$pdf->Cell(40,12,'Time_Allocated ');
...
if(mysqli_num_rows($result) > 0){
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $pdf->Cell(40,12,$row['agent_account_number'] ,1 ,0);
        $pdf->Cell(40,12,$row['lent_amount'],1 ,0);
        $pdf->Cell(40,12,$row['time_allocated'] ,1 ,0);
